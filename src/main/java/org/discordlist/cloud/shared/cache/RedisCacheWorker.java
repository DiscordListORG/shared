...
/**
 * RedisCache implementation of {@link com.mewna.catnip.cache.EntityCacheWorker} may contains some
 * code of {@link com.mewna.catnip.cache.MemoryEntityCache}
 */
...
public class RedisCacheWorker implements EntityCacheWorker {
  ...
  public RedisCacheWorker(RedisSource redisSource) {
    ...
    this.channelCache = new GuildSpecificSnowflakeRedisCache<>(
        ...
        (channel) -> channel.toJson().put("type", channel.type().getKey()).encode()
            .getBytes(StandardCharsets.UTF_8)
    );
    this.voiceStateCache = new GuildSpecificRedisCacheImpl<>(
        (state) -> new GuildSpecificRedisCache.GuildInformationContainer<>(state.guildIdAsLong(),
            state.userId()),
  ...
  @Nonnull
  @Override
  public Future<java.lang.Void> updateCache(@Nonnull String eventType, int shardId,
    ...
    switch (eventType) {
      ...
      case Raw.READY: {
        var future = CompletableFuture.supplyAsync(() -> {
          try (Jedis redis = redisSource.jedis()) {
            redis.set("selfUser",
                entityBuilder.createUser(payload.getJsonObject("user")).toJson().encode());
          }
          return null;
        }, executor);
      ...
      case Raw.CHANNEL_UPDATE: {
        ...
        if (channel.isGuild()) {
          ...
          log.warn(
              "I don't know how to cache channel {}: isCategory={}, isDM={}, isGroupDM={}," +
                  "isGuild={}, isText={}, isUserDM={}, isVoice={}",
              channel.idAsLong(), channel.isCategory(), channel.isDM(), channel.isGroupDM(),
              channel.isGuild(),
      ...
      case Raw.CHANNEL_DELETE: {
        final var channel = entityBuilder.createChannel(payload);
        if (channel.isGuild()) {
          return future(channelCache
              .delete(shardId, channel.asGuildChannel().guildIdAsLong(), channel.idAsLong()));
        } else if (channel.isUserDM()) {
      ...
      case Raw.GUILD_MEMBER_UPDATE: {
        ...
        if (old != null) {
          ...
          log.warn("Got GUILD_MEMBER_UPDATE for {} in {}, but we don't have them cached?!", id,
              guild);
        }
        break;
      }
      ...
      case Raw.GUILD_MEMBER_ADD: {
        ...
        return future(memberCache.cache(shardId, member),
            userCache.cache(shardId, user));
      }
      case Raw.GUILD_MEMBERS_CHUNK: {
        final var guild = payload.getString("guild_id");
        final JsonArray members = payload.getJsonArray("members");
        return future(members.stream().map(e -> memberCache.cache(Long.parseLong(guild), shardId,
            entityBuilder.createMember(guild, (JsonObject) e))).collect(Collectors.toList())
      ...
      case Raw.GUILD_EMOJIS_UPDATE: {
        if (!catnip.cacheFlags().contains(CacheFlag.DROP_EMOJI)) {
          ...
          return future(emojis.stream().map(e -> emojiCache.cache(guild, shardId,
              entityBuilder.createCustomEmoji(String.valueOf(guild), (JsonObject) e)))
  ...
  public NamedCacheView<Member> members(long guildId) {
    return memberCache
        .asNamedCacheView(guildId, (member) -> member.nick() == null ? "null" : member.nick());
  }
